...
const sourceDir = path.join(__dirname, '..', '..', 'frontend', 'public', 'materials', 'TCS');
const targetDir = path.join(__dirname, '..', '..', 'frontend', 'public', 'materials', 'TCS_text');
...
async function processDirectory(dir, currentRelativePath = '') {
    ...
    for (const file of files) {
        ...
        if (stat.isDirectory()) {
            ...
            const webPath = '/materials/TCS_text/' + (currentRelativePath ? currentRelativePath.replace(/\\/g, '/') + '/' : '') + newFileName;
...
async function run() {
    ...
    console.log('Finished extraction! Wrote index.json');
}
