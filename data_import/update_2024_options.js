...
async function updateAll2024() {
  ...
  console.log(`Re-processing all 36 papers of SSC CGL 2024 to restore numeric options...`);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    console.log(`[${paperNum}/36] Updating "${title}"...`);
    ...
    for (let s = 1; s <= 4; s++) {
      ...
      for (let j = 0; j < qBlocks.length - 1; j++) {
        ...
        // Only replace if literally empty or whitespace (pure image option)
        const isDiagramOpt = (str) => !str || str.trim().length === 0;
    ...
    console.log(`Successfully restored ${title}!`);
  }
  console.log("ALL 36 PAPERS RESTORED SUCCESSFULLY!");
}

updateAll2024().catch(console.error);
